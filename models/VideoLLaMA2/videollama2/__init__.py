...
def model_init(model_path=None, max_frames_num=NUM_FRAMES, **kwargs):
    model_path = "DAMO-NLP-SG/VideoLLaMA2-7B" if model_path is None else model_path
    model_name = get_model_name_from_path(model_path)
    tokenizer, model, raw_processor, context_len = load_pretrained_model(model_path, None, model_name, **kwargs)

    if tokenizer.pad_token is None and tokenizer.unk_token is not None:
        tokenizer.pad_token = tokenizer.unk_token

    # num_frames = model.config.num_frames if hasattr(model.config, "num_frames") else NUM_FRAMES
    num_frames = model.config.num_frames if hasattr(model.config, "num_frames") else max_frames_num
    print("num_frames:", num_frames)

    processor = {
        'image': partial(process_image, processor=raw_processor, aspect_ratio=None),
        'video': partial(process_video, processor=raw_processor, aspect_ratio=None, num_frames=num_frames),
    }

    return model, processor, tokenizer, raw_processor
