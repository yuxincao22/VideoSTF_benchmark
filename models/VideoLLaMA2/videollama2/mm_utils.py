...
def load_video_sole(video_path, s=None, e=None, num_frames=NUM_FRAMES):
    if isinstance(video_path, str):
        if s is not None and e is not None:
            s = s if s >= 0. else 0.
            e = e if e >= 0. else 0.
        ...
        if os.path.isdir(video_path):
            ...
            video_data = vreader.get_batch(sampled_frame_indices).asnumpy()
...
def process_video_sole(video_data, processor, aspect_ratio='pad'):
    if aspect_ratio == 'pad':
        images = [expand2square(f, tuple(int(x*255) for x in processor.image_mean)) for f in video_data]
        video = processor.preprocess(images, return_tensors='pt')['pixel_values']
    else:
