        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        llava_cfg = LlavaQwenConfig.from_pretrained(model_path, cache_dir=cache_dir)
                        rank0_print(f"Overwriting config with {overwrite_config}")
                        if model_path == "lmms-lab/LLaVA-NeXT-Video-32B-Qwen":
                            from transformers.models.qwen2 import Qwen2Config
                            if hasattr(llava_cfg, "text_config") and isinstance(llava_cfg.text_config, dict):
                                llava_cfg.text_config = Qwen2Config(**llava_cfg.text_config)
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
